...
const ProcessCard = ({ 
  ...
  const displayName = process.customLabel || process.processType || 'Unknown Process'
    ...
    >
      ...
      <div className="mb-4">
        ...
        <div className="text-sm text-app-muted">
          <span className="inline-block bg-app-panel/50 px-2 py-1 rounded">
            {process.processType}
          </span>
        </div>
      </div>
        ...
        <div className="space-y-1 text-sm">
          ...
          {Object.keys(process.parameters || {}).length > 0 && (
            <div className="text-app-text">
              <span className="text-app-muted">Parameters: </span>
              {Object.entries(process.parameters).slice(0, 2).map(([key, value]) => (
                <span key={key} className="mr-2">
                  {key}: {value}
                </span>
              ))}
              {Object.keys(process.parameters).length > 2 && (
                <span className="text-app-muted">+{Object.keys(process.parameters).length - 2} more</span>
              )}
            </div>
          )}
          ...
          {process.materials && process.materials.length > 0 && (
            <div className="text-app-text">
              <span className="text-app-muted">Materials: </span>
              {process.materials.slice(0, 2).map((material, idx) => (
                <span key={idx} className="mr-2">
                  {material.name} ({material.amount}kg)
                </span>
              ))}
              {process.materials.length > 2 && (
                <span className="text-app-muted">+{process.materials.length - 2} more</span>
              )}
            </div>
          )}
          ...
          {process.waters && process.waters.length > 0 && (
            <div className="text-app-text">
              <span className="text-app-muted">Solvents: </span>
              {process.waters.slice(0, 2).map((water, idx) => (
                <span key={idx} className="mr-2">
                  {water.name} ({water.volumeL}L)
                </span>
              ))}
              {process.waters.length > 2 && (
                <span className="text-app-muted">+{process.waters.length - 2} more</span>
              )}
            </div>
          )}
        </div>
      ...
      {isHovered && !isSelected && (
        <div className="absolute top-2 left-2 text-xs text-app-primary bg-app-primary/10 px-2 py-1 rounded">
          Click to select
        </div>
      )}
    </div>
